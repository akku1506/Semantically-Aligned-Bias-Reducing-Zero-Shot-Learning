...
parser.add_argument('--model_path', default='./logs_classifier_now/models_381.ckpt', type=str)
...
parser.add_argument('--conditional_modeldir', default='./models_cub', help='folder to get classifier model checkpoints')
parser.add_argument('--generator_checkpoint', type=int, default=69, help='tells which ckpt file of tensorflow model to load')
...
parser.add_argument('--nclass_all', type=int, help='number of all classes')
parser.add_argument('--regulariser', type=float, default=0.002, help='regulariser parameters for similarity with unseen classes')
...
with g1.as_default():
    ...
    noise_unseen_z = tf.placeholder(tf.float32,[opt.batch_size, opt.nz],name='noise_unseen')
    input_unseen_att = tf.placeholder(tf.float32,[opt.batch_size, opt.attSize],name='input_unseen_attributes')
    input_unseen_res = tf.placeholder(tf.float32,[opt.batch_size, opt.resSize],name='input_unseen_features')
    ...
    gen_res = generator(noise,opt,name='cond_gen',isTrainable=False,reuse=reuse)
    ...
    targetDisc = discriminator(targetEmbd,opt,name='cond_disc',isTrainable=False,reuse=reuse)
    genTargetEmbd = tf.concat([gen_res,input_att], axis=1)
    genTargetDisc = discriminator(genTargetEmbd,opt,name='cond_disc',isTrainable=False, reuse=True)
    ############################################################    

    noise = tf.concat([noise_unseen_z, input_unseen_att], axis=1)
    gen_unseen_res = generator(noise,opt,name='gen',isTrainable=train,reuse=reuse)

    unseenTargetDisc = discriminator(input_unseen_res,opt,name='disc',isTrainable=train,reuse=reuse)
    genUnseenTargetDisc = discriminator(gen_unseen_res,opt,name = 'disc',isTrainable=train, reuse=True)
    ...
    gradients = tf.gradients(discriminator(interpolate,opt,name='cond_disc',reuse=True,isTrainable=False), [interpolates])[0]
    ...
    genDiscMean = tf.reduce_mean(genUnseenTargetDisc)
    targetDiscMean = tf.reduce_mean(unseenTargetDisc)
    discriminatorUnseenLoss = tf.reduce_mean(genUnseenTargetDisc - unseenTargetDisc)
    alpha = tf.random_uniform(shape=[opt.batch_size,1], minval=0.,maxval=1.)

    interpolates = alpha * input_unseen_res + ((1 - alpha) * gen_unseen_res)
    #interpolate = tf.concat([interpolates, input_att], axis=1)
    gradients = tf.gradients(discriminator(interpolates,opt,name='disc',reuse=True,isTrainable=train), [interpolates])[0]
    slopes = tf.sqrt(tf.reduce_sum(tf.square(gradients), reduction_indices=[1]))
    gradientPenalty = tf.reduce_mean((slopes-1.)**2) 
        
    gradientPenalty = opt.lambda1*gradientPenalty
    discriminatorUnseenLoss = discriminatorUnseenLoss + gradientPenalty

    ################################################################3
    generatorUnseenLoss = -genDiscMean
    
    #################### getting parameters to optimize ####################
    discParams = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='cond_disc')
    discUnseenParams = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='disc')
    generatorParams = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='cond_gen')
    generatorUnseenParams = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='gen')
    loss_L2 = 0
    for params in generatorParams:
        for unseen_params in generatorUnseenParams:
            if (('gen_fc1' in params.name and 'gen_fc1' in unseen_params.name) or \
                ('gen_fc2' in params.name and 'gen_fc2' in unseen_params.name)) and \
                ('bias' not in params.name) and ('bias' not in unseen_params.name):
                #print (params,unseen_params)
                #print ('......')
                loss_L2 = loss_L2 + tf.nn.l2_loss(params-unseen_params)
    loss_L2 = loss_L2*opt.regulariser
    generatorTotalUnseenLoss = generatorUnseenLoss + loss_L2
    ...
    for params in discUnseenParams:
        print (params.name)
        print ('...................')
    ...
    for params in generatorUnseenParams:
        print (params.name)
        print ('...................')

    ##################################################################################

    discUnseenOptimizer = tf.train.AdamOptimizer(learning_rate=opt.lr,beta1=opt.beta1,beta2=0.999)
    genUnseenOptimizer = tf.train.AdamOptimizer(learning_rate=opt.lr, beta1=opt.beta1,beta2=0.999)

    discUnseenGradsVars= discUnseenOptimizer.compute_gradients(discriminatorUnseenLoss,var_list=discUnseenParams) 
    genUnseenGradsVars = genUnseenOptimizer.compute_gradients(generatorTotalUnseenLoss,var_list=generatorUnseenParams)


    discUnseenTrain = discUnseenOptimizer.apply_gradients(discUnseenGradsVars)
    generatorUnseenTrain = genUnseenOptimizer.apply_gradients(genUnseenGradsVars)

    #################### what all to visualize  ############################
    tf.summary.scalar("DiscriminatorLoss",discriminatorLoss)
    tf.summary.scalar("DiscriminatorUnseenLoss",discriminatorUnseenLoss)
    ...
    tf.summary.scalar("GeneratorTotalUnseenLoss",generatorTotalUnseenLoss)
    tf.summary.scalar("GeneratorUnseenLoss",generatorUnseenLoss)
    tf.summary.scalar("L2Loss",loss_L2)



    for g,v in discUnseenGradsVars:    
        tf.summary.histogram(v.name,v)
        tf.summary.histogram(v.name+str('grad'),g)

  
    for g,v in genUnseenGradsVars:    
        tf.summary.histogram(v.name,v)
        tf.summary.histogram(v.name+str('grad'),g)
    ...
    for name in params1:
        ...
        for i, s in enumerate(params):
                if new_name in s.name:
                    ...
                    sess.run(a)
                    print ('Model_loaded')
    params = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='cond_disc')
    generatorParams = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='cond_gen')
    for var in generatorParams:
        params.append(var)
    for var in params:
        print (var.name)
    print ('......................................')
    saver = tf.train.Saver(var_list=params)
    string = str(opt.conditional_modeldir)+'/models_'+str(opt.generator_checkpoint)+'.ckpt'
    #print (string) 
    try:
        ...
        for i in range(0, data.ntrain, opt.batch_size):
            ...
            for j in range(opt.critic_iter):
                ...
                fd = {  input_res:batch_feature,\
                        input_unseen_res:batch_unseen_feature,\
                        input_att:batch_att,\
                        input_unseen_att:batch_unseen_att,\
                        input_label:batch_label,\
                        noise_z:z_rand, \
                        noise_unseen_z:z_unseen_rand \
                    }


                _,discLoss,merged = sess.run([discUnseenTrain ,discriminatorUnseenLoss ,merged_all],feed_dict=fd)
            ...
            _,genLoss,merged = sess.run([generatorUnseenTrain,generatorTotalUnseenLoss,merged_all],feed_dict=fd)                
...
with g2.as_default():
    ...
    gen_res = generator(noise1,opt,name='gen',isTrainable=False,reuse=False)



############ getting features from g2 grakkph ############################3
...
with tf.Session(graph = g2) as sess:
    ...
    saver = tf.train.Saver()
    params = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='gen')
...
if opt.gzsl:
    ...
    train_cls = classifier2.CLASSIFICATION2(syn_res, util.map_label(syn_label, data.unseenclasses), data, data.unseenclasses.shape[0], 'logs_zsl_classifier','models_zsl_classifier', opt.classifier_lr , 0.5, 100, 100, False)
    acc = train_cls.acc
